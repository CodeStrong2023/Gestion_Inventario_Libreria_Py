...
from Backend.services.service_libro import LibroService
from Backend.services.service_revista import RevistaService
from unidecode import unidecode


class Ui_FormArt(object):
############################################## - - - CONFIG INICIALES
    def __init__(self, conn):
            self.conn = conn
            self.entidad_service = ArticuloService(conn)
        
            self.libro_service = LibroService(conn)
            self.revista_service = RevistaService(conn)
    
    #metodos de entidad principal
    def definir_modelo(self, entidad):
        modelo =[(str(entidad['idArticulo'])),(str(entidad['alto'])),(str(entidad['ancho'])),(str(entidad['peso'])),(str(entidad['precio'])),(entidad['tipo'])]
        
        return modelo
    
    def definir_modelo_carga(self, entidad, libro, revista):
        
        if entidad['tipo'] == "Libro":
            modelo = [(str(entidad['idArticulo'])),
            entidad['tipo'],
            libro.titulo,
            str(libro.anio),
            libro.isbn,
            (str(libro.Genero_idGenero)),
            (str(libro.Editorial_idEditorial)),
            
            libro.imagen_tapa,
            str(libro.edicion),
            str(libro.edad_minima),
            str(libro.edad_maxima),
            libro.idioma,
            libro.tapa,
            libro.descripcion,
            str(libro.paginas),
        ...
        #"ID", "Tipo", "Titulo", "Año", "ISBN", "Género","Editorial","Autor","Imagen","Edición / N°", "Edad Mínima", "Edad Máxima", "Idioma", "Tipo Tapa", "Descripción", "Páginas", "Alto","Ancho","Peso", "Precio"
        elif entidad['tipo'] == "Revista":
            modelo = [(str(entidad['idArticulo'])),
            entidad['tipo'],
            revista.titulo,
            str(revista.anio),
            "", #isbn
            revista.Genero_idGenero, 
            revista.Editorial_idEditorial,
    ...
    def entidad_crear(self, valores):
        resultado = self.entidad_service.crear_articulo(valores["alto"], valores["ancho"], valores["peso"], valores["precio"], valores["tipo"])
        
        return resultado
    
    def entidad_actualizar(self, idEntidad, valores):
        resultado = self.entidad_service.actualizar_articulo(idEntidad, valores["alto"], valores["ancho"], valores["peso"], valores["precio"], valores["tipo"])
        return resultado
    
    def entidad_buscar_todos(self):
        resultado = self.entidad_service.buscar_articulos()
        
        return resultado
    
    def entidad_buscar_generico(self, criterio_busqueda):
        resultado = self.entidad_service.buscar_articulos_generico(criterio_busqueda)
        return resultado
    
    def entidad_buscar_por_id(self, entidad_id):
        resultado = self.entidad_service.buscar_articulo(entidad_id)
        return resultado
    
    def entidad_eliminar(self, entidad_id):
        resultado = self.entidad_service.eliminar_articulo(entidad_id)
        return resultado      
...
############################################## - - - UI            
    ...
    #Variables de la pestaña
        ...
        self.nombre_tab = "ARTICULOS" 
        self.sing = "Articulo"
        self.plural = self.sing+"s"
        self.columnas = ["ID", "Tipo", "Titulo", "Año", "ISBN", "Género","Editorial","Imagen","Edición / N°", "Edad Mínima", "Edad Máxima", "Idioma", "Tipo Tapa", "Descripción", "Páginas", "Alto","Ancho","Peso", "Precio" ]
        self.columnas.extend([ "Eliminar"])
        
        
        self.campos_a_mostrar = ["Alto", "Ancho", "Peso", "Precio", "Tipo"]
...
# Widget para libro
        self.widget_crear_nuevo_2 = Widget().createWidget("widget_tipo_articulo")
        self.stacked_widget.addWidget(self.widget_crear_nuevo_2)
...
##  Principal - Tabla
    ...
    #crear la tabla 
        ...
        #ajustar ancho de columnas a contenido
        self.tabla_usuarios.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
...
##  Crear Nuevo
        ...
        for campo in self.campos_a_mostrar:
            ...
            if campo.lower() == "tipo": 
                tipos = ["Libro", "Revista"]
                #insertar combobox Libro o Revista

                self.input_n = comboBox().comboBoxPaises(tipos,self.seleccionar_pais)
                self.label_n = comboBox().labelForm(campo, self.palette.darkTextColor)
        ...
        self.boton_crear_empresa_form.clicked.connect(lambda: self.manejar_tipo_articulo(1,self.campos_texto))

##  Crear Nuevo Tipo de Articulo


        self.area_formulario_crear_2 = frame().frame(self.widget_crear_nuevo_2, 0, 0, 0, 0, "background-color: " + self.palette.lighBackgroundColor2 + ";", "area_formulario_crear")
        self.widget_crear_nuevo_2.setLayout(QtWidgets.QVBoxLayout(self.widget_crear_nuevo_2))

        self.formulario_layout_2 = QtWidgets.QFormLayout(self.area_formulario_crear_2)
        ...
        self.campos_a_mostrar_2 = ["Uno", "Dos", "Tres", "Precio", "Tipo"]
        self.campos_texto_2 = {}
        for campo in self.campos_a_mostrar_2:
            
            #Crear campo
                self.label_n = inputText().labelForm(campo, self.palette.darkTextColor)
                self.input_n = inputText().inputForm(self.area_formulario_crear_2, 180, 30, 60, 20, campo, self.palette.darkTextColor, self.palette.backgroundColor1, self.sing)
                    #Insertar campo
                self.formulario_layout_2.addRow(self.label_n, self.input_n)
                self.campos_texto_2[campo.lower()] = self.input_n
            
        

        # Botón Crear
        self.boton_crear_empresa_form_2 = boton().boton(self.area_formulario_crear_2, "background-color: rgb(85, 255, 127); padding: 10px; color:"+self.palette.darkTextColor+"; font-weight: bold;", "Crear "+self.sing, 120, 400, 101, 41, "boton_crear_empresa_form")
        self.boton_crear_empresa_form_2.setFixedSize(150, 50)
        self.formulario_layout_2.addRow("", self.boton_crear_empresa_form_2)

        self.formulario_layout_2.setContentsMargins(250, 100, 250, 100)  

        # Asegúrate de que el layout del formulario se aplique correctamente al widget principal
        self.area_formulario_crear_2.setLayout(self.formulario_layout_2)

        # Agregar el área del formulario al layout principal del widget_crear_nuevo
        self.widget_crear_nuevo_2.layout().addWidget(self.area_formulario_crear_2)
        
        #boton de formulario de creacion
        self.boton_crear_empresa_form_2.clicked.connect(lambda: self.manejar_tipo_articulo(3, self.campos_texto_2))
...
##  Actualizar
    ...
    def seleccionar_pais(self, index):
        pais_seleccionado = self.input_n.currentText()
...
#Funciones de vista
    ...
    def manejar_tipo_articulo(self, index, campos):
        print(f"manejar_tipo_articulo llamado con index: {index}")
        if index == 1 :
            self.crearEntidad(campos)
    ...
    def actualizarTablaEntidades(self, entidades):
        ...
        for row_idx, entidad in enumerate(entidades):
            ...
            if entidad['tipo'] == "Libro":
                libro = self.libro_service.leer_libros_articulo(entidad['idArticulo'])
                revista = None
                datos_entidad = self.definir_modelo_carga(entidad, libro, revista)
            elif entidad['tipo'] == "Revista":
                libro = None
                revista = self.revista_service.leer_revistas_articulo(entidad['idArticulo'])
                datos_entidad = self.definir_modelo_carga(entidad, libro, revista)
    ...
    def setearLista(self):
       self.tipo_elegido = self.input_n.currentText()
...
#Funciones de servicio
    ...
    def cargarEntidad(self):
        ...
        for row_idx, entidad in enumerate(entidades):
            ...
            if entidad['tipo'] == "Libro":
                libro = self.libro_service.leer_libros_articulo(entidad['idArticulo'])
                revista = None
                modelo = self.definir_modelo_carga(entidad, libro, revista)
            elif entidad['tipo'] == "Revista":
                libro = None
                revista = self.revista_service.leer_revistas_articulo(entidad['idArticulo'])
                modelo = self.definir_modelo_carga(entidad, libro, revista)
        ...
        # Ajustar el tamanio de las columnas
        ...
        self.tabla_usuarios.setStyleSheet(f"""
                                            ...
                                            QHeaderView::section {{
                                                ...
                                                font-size: 12pt;  /* Tamanio de la fuente */
                                            }}
                                        """)
    ...
    def ir_actualizar_entidad(self, row_idx):
        ...
        libro = self.libro_service.buscar_libro(entidad_data.idArticulo)
        revista = self.revista_service.buscar_revista(entidad_data.idArticulo)
    ...
    def confirmar_actualizacion(self, campos_texto):
        ...
        try:
            ...
            self.entidad_actualizar(self.idEntidad, valores)
