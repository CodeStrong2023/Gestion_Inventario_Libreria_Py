...
class Ui_FormAmacenf(object):
    def __init__(self, conn):
            self.conn = conn
            self.entidad_service = AlmacenamientoService(conn)
            self.direccion_service = DireccionService(conn)
            self.empresa_service = EmpresaService(conn)
            
    
    #metodos de entidad principal
    def definir_modelo(self, entidad, direccion, empresa):
        modelo = [
            (str(entidad['idAlmacenamiento'])),
            (entidad['capacidadTotalM3']),
            (entidad['usoM3']),
            (str(empresa.idEmpresa)),
            (str(direccion.idDireccion))
        ]
        
        return modelo
    def definir_modelo_carga(self, entidad, direccion, empresa, pais,  ciudad,calle, numero):
        modelo = [(str(entidad['idAlmacenamiento'])),
            (entidad['capacidadTotalM3']),
            (entidad['usoM3']),
            (pais.nombre),
            (ciudad.nombre),
    ...
    def entidad_buscar_todos(self):
        resultado = self.entidad_service.buscar_almacenamientos()
        return resultado
    
    def entidad_buscar_generico(self, criterio_busqueda):
        resultado = self.entidad_service.buscar_almacenamientos_generico(criterio_busqueda)
        return resultado
    
    def entidad_buscar_por_id(self, entidad_id):
        resultado = self.entidad_service.buscar_almacenamiento(entidad_id)
        return resultado
    
    def entidad_crear(self, valores, direccion_id, empresa_id):
        resultado = self.entidad_service.crear_almacenamiento(valores["capacidad"], valores["uso"], empresa_id, direccion_id)        
        return resultado
    
    def entidad_actualizar(self, idEntidad, valores, direccion_id, empresa_id):
        resultado = self.entidad_service.actualizar_almacenamiento(idEntidad, valores["capacidad"], valores["uso"], empresa_id, direccion_id)
        return resultado
    
    def entidad_eliminar(self, entidad_id):
        resultado = self.entidad_service.eliminar_almacenamiento(entidad_id)
        return resultado       
    
    def setupUi(self, FormAmacenf, palette):
        self.formulario = FormAmacenf
        self.nombre_app = "Gestor de Biblioteca"
        self.nombre_tab = "ALMACENES"
        self.sing = "Almacen"
        self.segundoP = "Articulo"
        self.plural = self.sing+"s"
        self.columnas = ["ID", "Capacidad", "Uso", "Pais", "Ciudad", "Calle", "Numero", "Empresa"]
        self.columnas.extend(["Actualizar", "Eliminar"])
        
        self.campos_a_mostrar = ["Capacidad", "Uso", "Pais", "Ciudad", "Calle", "Numero", "Empresa"]
...
##  Crear Nuevo
        ...
        for campo in self.campos_a_mostrar:
            if campo == "Pais" or campo == "Ciudad" or campo == "Calle" or campo == "Numero"  or campo == "Capacidad" or campo == "Uso":
            #Crear campo
...
#Funciones de vista
    ...
    def actualizarTablaEntidades(self, entidades):
        ...
        for row_idx, entidad in enumerate(entidades):
            ...
            datos_entidad = self.definir_modelo_carga(entidad, direccion, empresa, pais, ciudad, direccion.calle, direccion.numero)
...
#Funciones de servicio
    ...
    def cargarEntidad(self):
        ...
        for row_idx, entidad in enumerate(entidades):
            ...
            modelo = self.definir_modelo_carga(entidad, direccion, empresa, pais, ciudad, direccion.calle, direccion.numero)
            #print(modelo)
    ...
    def confirmar_actualizacion(self, campos_texto):
        ...
        try:
            ...
            empresa_id = 1
            self.entidad_actualizar(self.idEntidad, valores, direccion_id, empresa_id)
            empresa_id = empresa_id.Empresa_idEmpresa
    ...
    def crearEntidad(self, campos_texto):
        ...
        direccion_id = self.devolver_direccion(valores["ciudad"], valores["pais"], valores["calle"], valores["numero"])
        empresa_id = 1
        self.entidad_crear(valores, direccion_id, empresa_id)
